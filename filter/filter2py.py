    ...
    for el in iterable:
        ...
        if name in ("h1","h2","h3","h4","h5","h6"):
            ...
            if raw and (name == "th" or el.find(["b","strong"])):
                txt = raw
                heading_level = 3  # treat as mid-level heading
            ...
            txt = el.get_text()  # keep formatting
...
def main():
    ap = argparse.ArgumentParser(description="Filter HTML file to Markdown-like text.")
...
# ---------- parse filtered Markdown ----------
def parse_filtered_markdown(txt: str):
    """
    Extract metadata (title, h1) and keep text.
    Expects format like:
      title:Something
      # Page H1
      ## Section
    ...
    for ln in lines:
        if ln.startswith("title:") and not title:
            ...
            h1 = ln[2:].strip()
            cleaned_lines.append(ln)  # keep H1 in doc
        else:
...
def split_sections(md_text: str):
    """
    Splits on Markdown headings (## or deeper).
    Returns list of dicts: {heading, level, text}
    """
    lines = md_text.splitlines()
    sections = []
    cur = {"heading": None, "level": 0, "text": []}
    for ln in lines:
...
def chunk_text_with_overlap(text: str, target_tokens=700, overlap_tokens=80):
    if not text:
        return []
    # protect <pre> blocks: treat as indivisible
...
def pack_to_jsonl(filtered_text: str, source_path: str, out_path: str):
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        for i, sec in enumerate(sections):
            ...
            hp = build_heading_path(sections, i)
            anchor = slug(heading)
